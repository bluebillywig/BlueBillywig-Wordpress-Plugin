...
function bb_mediaclip_shortcode($userAtts)
{
	...
	if (!array_key_exists('playout', $atts) || empty($atts['playout'])) {
		$atts['playout'] = 'default';
	}

	$useIframe = get_option(BB_PLUGIN_SETTING_USE_IFRAME_EMBED);
	...
	if ($useIframe === '1') {
		...
		$output .= '.html?inheritDimensions=true" width="720" height="405"  frameborder="0" webkitallowfullscreen mozallowFullscreen oallowFullscreen msallowFullscreen allowfullscreen allow="autoplay; fullscreen"></iframe>';
	} else {
...
function search_videos_request()
{
	...
	if (isset($_REQUEST)) {
		...
		if (array_key_exists('status', $_REQUEST)) {
			$properties['query'] .= ' AND status:"' . $_REQUEST['status'] . '"';
		}
...
function build_mediaclip_preview($mediaclip, $options, $width = 300, $height = 150, $clickAction = 'selectClip', $class = 'bb-thumbnail-wrapper')
{
	...
	$clipTitle = isset($mediaclip->title) ? $mediaclip->title : (is_array($mediaclip) && isset($mediaclip['title']) ? $mediaclip['title'] : 'Untitled video');
	$clipThumbnail = isset($mediaclip->mainthumbnail_string) ? $mediaclip->mainthumbnail_string : (is_array($mediaclip) && isset($mediaclip['mainthumbnail_string']) ? $mediaclip['mainthumbnail_string'] : NULL);
...
<?php
		...
		function render_setting_boolean($title, $name, $value, $description = '')
		{
?>
	<tr>
		<th><span><?php echo $title ?> </span></th>
