<?php
    ...
    //iframe embed
    if(isset($_POST[BB_PLUGIN_SETTING_USE_IFRAME_EMBED])){
        $iframeEmbed = $_POST[BB_PLUGIN_SETTING_USE_IFRAME_EMBED];

        BlueBillywig::instance()->save_plugin_option(BB_PLUGIN_SETTING_USE_IFRAME_EMBED, $iframeEmbed);
    }
    ...
    $iframeEmbed = BlueBillywig::instance()->get_plugin_option(BB_PLUGIN_SETTING_USE_IFRAME_EMBED);
...
<?php
    render_form_start(BB_STRINGS['API_SETTINGS_TITLE']);
        ...
        render_setting_boolean( BB_STRINGS['SETTINGS_PLUGIN_USE_IFRAME_EMBED'], BB_PLUGIN_SETTING_USE_IFRAME_EMBED, $iframeEmbed,
        'Enable this to wrap the player inside an iframe when embedding a Blue Billywig video(Useful for RSS feeds or CMS\'s that block scripts)');
